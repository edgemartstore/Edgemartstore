...
import { Mail, Lock, LogIn, Loader2, Landmark } from "lucide-react";
...
export const Login: React.FC = () => {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === "auth/operation-not-allowed" || err.message?.includes("operation-not-allowed")) {
        setErrorMessage("EMAIL_PROVIDER_NOT_ENABLED: Email/Password sign-in is not enabled in the Firebase Console.");
      } else {
        setErrorMessage(err.message || "Failed to authenticate your email/password credentials.");
      }
    }
  };

  // Google OAuth Popup login configuration as recommended by skill
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === "auth/operation-not-allowed" || err.message?.includes("operation-not-allowed")) {
        setErrorMessage(
          "GOOGLE_PROVIDER_NOT_ENABLED: The Google Auth login provider is not yet enabled in the Firebase Console."
        );
      } else {
    ...
    <div className="max-w-md mx-auto px-4 py-24 flex flex-col items-center">
      ...
      <div className="w-full bg-[#111111] rounded-2xl border border-white/5 p-8 shadow-xl flex flex-col justify-between relative overflow-hidden text-white font-sans font-medium">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-black font-display uppercase text-white tracking-wider">Customer Sign In</h2>
          <p className="text-zinc-500 text-xs mt-1.5 leading-relaxed">Access and modify your secure grocery budget allocations</p>
        </div>
            ...
            {errorMessage.includes("GOOGLE_PROVIDER_NOT_ENABLED") ? (
              ...
              <div className="text-left space-y-2">
                ...
                <p className="text-[11px] text-zinc-400 font-normal">
                  💡 <em>You can also use <strong>Sign In with Google</strong> above if Google sign-in is configured.</em>
                </p>
              </div>
            ) : (
        ...
        {/* Primary Premium Google OAuth Login */}
        <button
        ...
        <p className="text-zinc-500 text-xs text-center mt-6">
          ...
          <Link to="/register" className="text-red-500 font-extrabold hover:underline">
